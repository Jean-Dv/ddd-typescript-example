...
describe("FindCoursesCounterQueryHandler", () => {
  ...
  it("should find an existing courses counter", async () => {
    ...
    expect(counter.total.value).toBe(response.total)
  })

  it("should throw an exception when courses counter does not exist", async () => {
    const finder = new CoursesCounterFinder(repository)
    const handler = new FindCoursesCounterQueryHandler(finder)

    await expect(
      handler.handle(new FindCoursesCounterQuery())
    ).rejects.toBeInstanceOf(CoursesCounterNotExist)
  })
})
